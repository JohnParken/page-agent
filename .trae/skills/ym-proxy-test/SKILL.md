---
name: ym-proxy-test
description: 验证 YmProxyServer 代理服务器的 init_session 和 chat 接口是否正常工作。当需要测试 Yiming Provider 代理功能、排查代理服务器问题、或验证 Qwen API 集成时调用此 skill。
---

# YmProxyServer 验证 Skill

## 概述

此 skill 用于验证 YmProxyServer 代理服务器的两个核心接口：
...
```bash
curl -X POST http://localhost:8089/chatabc/init_session \
...
# 3. 发送请求
curl -X POST http://localhost:8089/chatabc/chat \
  -H "Content-Type: application/json" \
  -d @request.json | jq .
```
...
# 或使用其他端口启动
PROXY_PORT=8089 npm run start:ym-proxy
```
