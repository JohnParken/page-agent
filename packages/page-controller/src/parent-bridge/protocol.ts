 ...
 *
 * The assistant requests the first handshake after an explicit application
 * action. The parent then signs and sends an offer, the assistant validates it
 * through its application-owned authorizer and sends an accept acknowledgement,
 * and the parent transfers a MessageChannel with a separate connect message.
 * An activated session may later be reconnected by either side.
 */
...
export interface VerifiedEmbedPolicyClaims {
	...
	exp: number
	/**
	 * Optional server-verified binding for integration-aware authorization.
	 * These values already exist in the wire offer; keeping the binding in the
	 * verified claims lets the Host prove that Auth approved that exact offer.
	 */
	bridgeBinding?: ParentControllerBridgeBinding
	/** Connection lifetime after the one-use policy exchange window closes. */
	bridgeSessionExp?: number
	/** Optional, verified grants for explicitly configured child iframe proxies. */
	childFrames?: VerifiedEmbedPolicyChildFrameGrant[]
	[key: string]: unknown
}

/** Handshake values generated by P before it requests an embed policy. */
export interface ParentControllerBridgeBinding {
	sessionId: string
	challenge: string
...
export type ParentControllerHandshakeReason = 'user' | 'reconnect'

/** Assistant bootstrap request. It contains no identity, policy, or business data. */
export interface ParentControllerHandshakeRequestMessage extends ParentControllerMessageBase {
	type: 'handshake-request'
	requestId: string
	reason: ParentControllerHandshakeReason
}

/** Assistant clears the in-memory activation lease on both sides. */
export interface ParentControllerDeactivateMessage extends ParentControllerMessageBase {
	type: 'deactivate'
	requestId: string
}

/** Parent sends this only after an assistant request or an activated reconnect. */
export interface ParentControllerOfferMessage extends ParentControllerMessageBase {
	type: 'offer'
	/** Echoed only for an assistant-initiated handshake. */
	handshakeRequestId?: string
	/** True only when P is reconnecting an already activated bridge session. */
...
export type ParentControllerWindowMessage =
	| ParentControllerHandshakeRequestMessage
	| ParentControllerDeactivateMessage
...
export function isParentControllerOfferMessage(
	...
	return (
		...
		hasProtocol(value) &&
		value.type === 'offer' &&
		(value.handshakeRequestId === undefined || isIdentifier(value.handshakeRequestId)) &&
		(value.automaticReconnect === undefined || typeof value.automaticReconnect === 'boolean') &&
...
export function isParentControllerHandshakeRequestMessage(
	value: unknown
): value is ParentControllerHandshakeRequestMessage {
	return (
		isRecord(value) &&
		hasOnlyKeys(value, ['protocol', 'version', 'type', 'requestId', 'reason']) &&
		hasProtocol(value) &&
		value.type === 'handshake-request' &&
		isIdentifier(value.requestId) &&
		(value.reason === 'user' || value.reason === 'reconnect') &&
		isParentControllerMessageSizeAllowed(value)
	)
}

export function isParentControllerDeactivateMessage(
	value: unknown
): value is ParentControllerDeactivateMessage {
	return (
		isRecord(value) &&
		hasOnlyKeys(value, ['protocol', 'version', 'type', 'requestId']) &&
		hasProtocol(value) &&
