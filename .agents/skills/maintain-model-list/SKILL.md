...
The supported model list and compatibility patches live in two places:

| File                                    | What it holds                                                                                                  |
| --------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| `packages/llms/src/models.live.test.ts` | `MODEL_GROUPS`, plus `OPENROUTER_ID_OVERRIDES` / `ALIYUN_ID_OVERRIDES` for provider-specific model ids        |
...
3. **Add the model to `MODEL_GROUPS`** in the live test, newest first within its brand group. Add id overrides if the OpenRouter/Aliyun id differs from the display name.
...
A failure means the request/response shape is wrong for that model — fix `modelPatch` or the id override, don't shrug it off. If the model fails consistently on a provider, exclude it from that provider (see the `deepseek-3.2` precedent in the test file) and document why in a comment.
...
4. **Act by significance**:
    - Minor updates (a preview id went stable, a small point-release replaces its predecessor): apply the change yourself following Workflow A steps 2–5, then present it to the user for judgment.
